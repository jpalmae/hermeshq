  ...
  es: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          "Requisitos previos: suscripción Microsoft 365 con permisos de administrador, Azure Portal para registrar aplicaciones.",
          ...
          "Paso 5 — Configurar webhook: la URL de webhook es `https://<tu-dominio>/webhooks/teams`. Registra esta URL en el Bot Framework portal como messaging endpoint.",
          "Paso 6 — Crear el secreto en HermesHQ: ir a Settings → Secrets. Crear un nuevo secreto con provider `microsoft_teams`. El campo username debe contener el Application (client) ID y el campo valor debe contener el client secret de Azure.",
          "Paso 7 — Configurar el canal: ir al agente → Mensajería → tab MS Teams. Seleccionar el secreto creado, habilitar el canal y guardar. Opcionalmente agregar `app_id` y `tenant_id` en metadata_json.",
          "El gateway de Teams usa OAuth2 con client_credentials para obtener tokens de acceso del Microsoft Identity Platform. Los tokens se renuevan automáticamente cada 5 minutos.",
          "Los mensajes entrantes se convierten en tareas del agente (source: microsoft_teams). Las respuestas se envían de vuelta a la conversación de Teams.",
          "Soporta mensajes 1:1 (chat privado), mensajes en canales de equipo y respuestas en hilos (threads).",
          "El estado del canal (running/stopped/error) es visible en el panel de mensajería del agente.",
          "Para producción se recomienda configurar el Bot Framework con HTTPS y validar el token JWT de Microsoft en el webhook.",
          "Para probar sin un agente corriendo: verifica que el webhook responda con `curl -X POST https://<tu-dominio>/webhooks/teams -H 'Content-Type: application/json' -d '{\"type\":\"ping\"}'`. Debe retornar `{\"status\":\"ok\"}`.",
        ],
      },
      {
        id: "google-chat",
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "Step 5 — Configure webhook: the webhook URL is `https://<your-domain>/webhooks/teams`. Register this URL in the Bot Framework portal as the messaging endpoint.",
          "Step 6 — Create the secret in HermesHQ: go to Settings → Secrets. Create a new secret with provider `microsoft_teams`. Put the Application (client) ID in the username field and the client secret Value in the value field.",
          "Step 7 — Configure the channel: go to agent → Messaging → MS Teams tab. Select the secret you created, enable the channel, and save. Optionally add `app_id` and `tenant_id` in metadata_json.",
          "The Teams gateway uses OAuth2 client_credentials to obtain access tokens from the Microsoft Identity Platform. Tokens are refreshed automatically every 5 minutes.",
          "Incoming messages are converted to agent tasks (source: microsoft_teams). Replies are sent back to the Teams conversation.",
          "Supports 1:1 messages (private chat), team channel messages, and threaded replies.",
          "Channel status (running/stopped/error) is visible in the agent messaging panel.",
          "For production, configure Bot Framework with HTTPS and validate the Microsoft JWT token at the webhook endpoint.",
          "To test without a running agent: verify the webhook responds with `curl -X POST https://<your-domain>/webhooks/teams -H 'Content-Type: application/json' -d '{\"type\":\"ping\"}'`. It should return `{\"status\":\"ok\"}`.",
        ],
      },
      {
        id: "google-chat",
