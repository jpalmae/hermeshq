  ...
  es: {
    ...
    sections: [
      ...
      },
      {
        ...
        summary:
          "Conecta un agente a Microsoft Teams como bot de 1:1 chat y canales de equipo. Los mensajes se enrutan como tareas al agente y las respuestas se publican automáticamente en la conversación.",
        audience: "Administradores",
        bullets: [
          "Requisitos previos: suscripción Microsoft 365 con permisos de administrador, Azure Portal acceso para registrar aplicaciones.",
          ...
          "Paso 4 — Crear canal Bot: en Azure Bot resource, vincular la App Registration y habilitar Microsoft Teams channel.",
          "Paso 5 — Configurar webhook: la URL de webhook es `https://<tu-dominio>/webhooks/teams`. Registra esta URL en el Bot Framework portal.",
          "Paso 6 — En HermesHQ: ir al agente → Mensajería → tab MS Teams. Habilitar, ingresar secret_ref con el client secret, y guardar.",
          "El gateway de Teams usa OAuth2 con client_credentials para obtener tokens de acceso del Microsoft Identity Platform.",
          "Los mensajes entrantes se convierten en tareas del agente (source: msteams_inbound). Las respuestas se envían de vuelta a la conversación de Teams.",
          "Soporta mensajes 1:1 (chat privado), mensajes en canales de equipo y respuestas en hilos (threads).",
          "El estado del canal (running/stopped/error) y los logs del gateway son visibles en el panel de mensajería del agente.",
          "Para producción se recomienda configurar el Bot Framework con HTTPS y validar el token JWT de Microsoft en el webhook.",
        ],
      },
      {
        id: "google-chat",
        ...
        bullets: [
          ...
          "Paso 5 — En HermesHQ: ir al agente → Mensajería → tab Google Chat. Habilitar, ingresar secret_ref con el contenido del JSON de la Service Account, y guardar.",
          "El gateway usa la Service Account para autenticarse via OAuth2 (scope chat.bot) y obtener access tokens para la REST API de Google Chat.",
          "Los mensajes entrantes (MESSAGE events) se convierten en tareas del agente (source: googlechat_inbound). Las respuestas se envían via la Chat API REST.",
          "Soporta mensajes directos (DM), espacios (group chats) y menciones del bot con @botname.",
          "El estado del canal y logs del gateway son visibles en el panel de mensajería del agente.",
  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        title: "Microsoft Teams",
        summary:
          "Connect an agent to Microsoft Teams as a bot for 1:1 chats and team channels. Messages are routed as tasks to the agent and replies are posted back to the conversation automatically.",
        audience: "Administrators",
        bullets: [
          "Prerequisites: Microsoft 365 subscription with admin access, Azure Portal access for app registration.",
          ...
          "Step 4 — Create Bot channel: in Azure Bot resource, link the App Registration and enable the Microsoft Teams channel.",
          "Step 5 — Configure webhook: the webhook URL is `https://<your-domain>/webhooks/teams`. Register this URL in the Bot Framework portal.",
          "Step 6 — In HermesHQ: go to agent → Messaging → MS Teams tab. Enable, enter the client secret as secret_ref, and save.",
          "The Teams gateway uses OAuth2 client_credentials to obtain access tokens from the Microsoft Identity Platform.",
          "Incoming messages are converted to agent tasks (source: msteams_inbound). Replies are sent back to the Teams conversation.",
          "Supports 1:1 messages (private chat), team channel messages, and threaded replies.",
          "Channel status (running/stopped/error) and gateway logs are visible in the agent messaging panel.",
          "For production, configure Bot Framework with HTTPS and validate the Microsoft JWT token at the webhook endpoint.",
        ],
      },
      {
        id: "google-chat",
        ...
        bullets: [
          ...
          "Step 5 — In HermesHQ: go to agent → Messaging → Google Chat tab. Enable, enter the Service Account JSON as secret_ref, and save.",
          "The gateway uses the Service Account to authenticate via OAuth2 (chat.bot scope) and obtain access tokens for the Google Chat REST API.",
          "Incoming MESSAGE events are converted to agent tasks (source: googlechat_inbound). Replies are sent via the Chat REST API.",
          "Supports direct messages (DM), spaces (group chats), and bot mentions with @botname.",
          "Channel status and gateway logs are visible in the agent messaging panel.",
